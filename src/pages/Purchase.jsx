...
function Purchase() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const purchaseResponse = await axios.get(
          `http://13.127.41.16:8080/api/erp/purchases/${userName}`,
          config
        );
        ...
        const purchaseOutstandingResponse = await axios.get(
          `http://13.127.41.16:8080/api/erp/purchaseOutstanding/${userName}`,
          config
        );
  ...
  const handlePurchaseIdChange = async (selectedOption) => {
    ...
    const getSelectedPurchaseResponse = await axios.get(
      `http://13.127.41.16:8080/api/erp/purchasesById/${newSelectedPurchaseId}/${userName}`
    );
  ...
  const handleSubmitPurchase = async (event) => {
    ...
    try {
      const purchaseAdd = await axios.post(
        "http://13.127.41.16:8080/api/erp/purchases/add",
        data,
        config
      );
      ...
      for (const item of selectedProductArray) {
        const inventoryPurchaseUdpate = await axios.put(
          `http://13.127.41.16:8080/api/erp/inventory/purchase/update/${item.product}/${userName}?quantity=${item.quantity}&purchase_price=${item.price}`,
          null,
          configForPut
        );
  ...
  const handleSubmitReturn = async (event) => {
    ...
    try {
      const inventoryUpdateResponse = await axios.put(
        `http://13.127.41.16:8080/api/erp/inventory/purchase/return/${selectedProductIdForReturn}/${userName}?quantity=${quantity}`,
        null,
        config
      );
      ...
      const purchaseUpdateResponse = await axios.put(
        `http://13.127.41.16:8080/api/erp/purchase/return/${selectedPurchaseId}/${selectedProductIdForReturn}/${userName}?returnQuantity=${quantity}&returnedAmount=${amount}`,
        null,
        config
      );
  ...
  const handlePartPayment = async (event) => {
    ...
    try {
      const response = await axios.put(
        `http://13.127.41.16:8080/api/erp/purchases/partPayment/${selectedPurchaseId}/${userName}?nextAdvance=${restOfPayment}`,
        null,
        configForPut
      );
