...
function MassInput() {
  ...
  const handleSubmit = async (e) => {
    ...
    axios
      .post("https://43.204.30.111:8080/api/erp/productList/add", updatedProductData)
      .then((response) => {
  ...
  const generateTable = () => {
    ...
    for (let i = 0; i < rows; i++) {
      ...
      inputRow.push(
        <td key="productName" style={{maxWidth:"180px"}}>
          <input
            type="text"
            name="productName"
            data-row={i}
      ...
      inputRow.push(
        <td key="description" style={{maxWidth:"180px"}}>
          <input
            type="text"
            name="description"
            min="0"
      ...
      inputRow.push(
        <td key="sellingPrice" style={{maxWidth:"180px"}}>
          <input
            type="number"
            name="sellingPrice"
            min="0"
      ...
      inputRow.push(
        <td key="existingStock" style={{maxWidth:"180px"}}>
          <input
            type="number"
            name="existingStock"
            min="0"
  ...
  return (
    <form style={{ paddingTop: "15px" }}>
      <div id="main" className="listing-container-ash">
        <div>
          <h3 style={{ textAlign: "center" }}>{t("enterStockDetails")}</h3>
            <table className="centered-table" style={{maxWidth:"500px"}}>
