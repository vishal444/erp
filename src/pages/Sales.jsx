...
function Sales() {
    ...
    async function fetchData() {
      try {
        ...
        const salesResponse = await axios.get(
          `https://bisbuddy.xyz:8080/api/erp/sales/${userName}`,
          config
        );
        ...
        const salesOutstandingResponse = await axios.get(
          `https://bisbuddy.xyz:8080/api/erp/salesOutstanding/${userName}`,
          config
        );
        ...
        const customersResponse = await axios.get(
          `https://bisbuddy.xyz:8080/api/erp/customer/getAll/${userName}`,
          config
        );
        setCustomers(customersResponse.data);
        const companyResponse = await axios.get(
          `https://bisbuddy.xyz:8080/api/erp/company/${userName}`,
          config
        );
  ...
  const handleSalesIdChange = async (selectedOption) => {
    ...
    const getSelectedSaleResponse = await axios.get(
      `https://bisbuddy.xyz:8080/api/erp/salesById/${newSelectedSalesId}/${userName}`
    );
  ...
  const handleSubmitSales = async (event) => {
    ...
    try {
      const addSaleResponse = await axios.post(
        "https://bisbuddy.xyz:8080/api/erp/sales/add",
        salesData,
        config
      );
      ...
      for (const item of selectedProductArray) {
        const updateInventoryResponse = await axios.put(
          `https://bisbuddy.xyz:8080/api/erp/inventory/sale/update/${item.product}/${userName}?quantity=${item.quantity}`,
          null,
          configForPut
        );
  ...
  const handleSubmitReturn = async (event) => {
    ...
    try {
      const updateInventoryResponse = await axios.put(
        `https://bisbuddy.xyz:8080/api/erp/inventory/sale/return/${selectedProductIdForReturn}/${userName}?quantity=${quantity}`,
        null,
        config
      );
      console.log(updateInventoryResponse.data);
      ...
      const updateSalesReturnResponse = await axios.put(
        `https://bisbuddy.xyz:8080/api/erp/sales/return/${selectedSalesId}/${selectedProductIdForReturn}/${userName}?returnQuantity=${quantity}&returnedAmount=${amount}`,
        null,
        config
      );
  ...
  const handleAdvancePayment = async (event) => {
    ...
    try {
      const response = await axios.put(
        `https://bisbuddy.xyz:8080/api/erp/sales/advance/${selectedSalesId}/${userName}?nextAdvance=${restOfAdvance}`,
        null,
        configForPut
      );
